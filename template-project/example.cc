...
int main()
{
    ...
    // Sprawdzenie typĂłw wynikĂłw wywoĹaĹ
    ...
    for (const auto& t : invoke_intseq([](auto a, auto b, int c, auto d) {return std::make_tuple(a, b, c, d);}, "ala", "ma", std::integer_sequence<int, 5, 7, 9>(), "kotĂłw"))
    ...
    std::cout << "Foo: auto&" << std::endl;
    // Nie powinno siÄ kompilowaÄ: rvalue nie moĹźe byÄ przekazywane jako auto&.
